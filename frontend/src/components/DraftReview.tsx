...
export function DraftReview({ session, onApproved, onDiscarded }: Props) {
  ...
  const critiqueIssues: string[] = session.self_critique
    ? (() => { try { return JSON.parse(session.self_critique); } catch { return []; } })()
    : [];
  const wasRevised = critiqueIssues.length > 0 && session.agent_draft_v1 !== session.agent_draft;
  const hasReasoning = !!(session.agent_draft_v1 || critiqueIssues.length > 0);
      ...
      {hasReasoning && (
        <div className="mx-6 mb-2">
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 hover:bg-slate-100 transition"
          >
            <span className="flex items-center gap-2">
              ...
              {!wasRevised && critiqueIssues.length === 0 && (
          ...
          {reasoningOpen && (
            <div className="mt-2 px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 space-y-3">
              {critiqueIssues.length > 0 ? (
                <Fragment>
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                    Issues the agent caught and fixed
                  </p>
                ...
                <p className="text-xs text-slate-500">
                  The agent was satisfied with the initial draft — no revisions were made.
                </p>
              )}
            </div>
          )}
